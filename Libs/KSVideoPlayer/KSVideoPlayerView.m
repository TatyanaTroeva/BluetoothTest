...
-(void)initializePlayer:(CGRect)frame
{
    ...
    self.closeButton.frame = CGRectMake(10, 25, 50, 20);
    ...
    self.playPauseButton.frame = frame;//CGRectMake(5*frameWidth/240, 6*frameHeight/160, 16*frameWidth/240, 16*frameHeight/160);
...
//    [self.playPauseButton setBackgroundImage:[UIImage imageNamed:@"avplayer.bundle/playback_pause"] forState:UIControlStateSelected];
    [self.playPauseButton setBackgroundImage:[UIImage imageNamed:@"pause_none.png"] forState:UIControlStateSelected];
...
- (IBAction)onClose:(id)sender{
    ...
    if ([self.delegate respondsToSelector:@selector(playerClose:)]) {
        [self.delegate playerClose:self];
    }
...
//    if ([self.delegate respondsToSelector:@selector(playerChangedPlayback:Time:)]) {
//        [self.delegate playerChangedPlayback:self Time:seekTime];
//    }
}

-(void) seekToTime:(CMTime)time
{
    [self.moviePlayer seekToTime:time];
...
-(void)play
{
    ...
    if ([self.delegate respondsToSelector:@selector(playerClickedPlay:)]) {
        [self.delegate playerClickedPlay:self];
    }
}
...
-(void)pause
{
    ...
    if ([self.delegate respondsToSelector:@selector(playerClickedPause:)]) {
        [self.delegate playerClickedPause:self];
    }
